...
prompt_2() {
    ...
    if [ $? -eq 0 ]; then
        ...
        sudo apt install -f --assume-yes --no-install-recommends dialog dosbox dxvk speedtest-cli steam lutris m4 make zenity winetricks
    fi
    which pacman >/dev/null 2>&1
    if [ $? -eq 0 ]; then
        ...
        yay -S --needed --noconfirm dialog dosbox dxvk-bin speedtest-cli steam lutris m4 make zenity winetricks gst-editing-services
    fi
    which dnf >/dev/null 2>&1
    if [ $? -eq 0 ]; then
        sudo dnf config-manager --add-repo https://dl.winehq.org/wine-builds/fedora/$(rpm -E %fedora)/winehq.repo &&
            sudo dnf install vulkan-loader winehq-staging -y
        sudo dnf install dialog dosbox speedtest-cli steam lutris m4 make zenity gst-editing-services -y
    fi
