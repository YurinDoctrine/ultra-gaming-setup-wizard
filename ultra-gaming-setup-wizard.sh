...
nvidia() {
    ...
    which pacman >/dev/null 2>&1
    if [ $? -eq 0 ]; then
        yay -S --needed --noconfirm mesa lib32-mesa nvidia nvidia-dkms opencl-nvidia lib32-opencl-nvidia nvidia-utils lib32-nvidia-utils xf86-video-nouveau libva-mesa-driver vulkan-tools vulkan-intel lib32-vulkan-intel vulkan-mesa-layers lib32-vulkan-mesa-layers mesa-vdpau lib32-mesa-vdpau &&
            echo -e "WINEPREFIX=~/.wine" | sudo tee -a /etc/environment &&
            echo -e "WINE_LARGE_ADDRESS_AWARE=1" | sudo tee -a /etc/environment &&
            echo -e "WINEFSYNC_SPINCOUNT=24" | sudo tee -a /etc/environment &&
            echo -e "WINEFSYNC=1" | sudo tee -a /etc/environment &&
...
prompt_2() {
    ...
    if [ $? -eq 0 ]; then
        ...
        sudo apt install -f --assume-yes --no-install-recommends dialog dosbox dxvk speedtest-cli steam hwloc lutris m4 make zenity winetricks
    fi
    which pacman >/dev/null 2>&1
    if [ $? -eq 0 ]; then
        ...
        yay -S --needed --noconfirm dialog dosbox dxvk-bin speedtest-cli steam hwloc lutris m4 make zenity winetricks gst-editing-services
    fi
    which dnf >/dev/null 2>&1
    if [ $? -eq 0 ]; then
        sudo dnf config-manager --add-repo https://dl.winehq.org/wine-builds/fedora/$(rpm -E %fedora)/winehq.repo &&
            sudo dnf install vulkan-loader winehq-staging -y
        sudo dnf install dialog dosbox speedtest-cli steam hwloc lutris m4 make zenity gst-editing-services -y
    fi
