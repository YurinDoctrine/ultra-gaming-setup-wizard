...
prompt_2() {
    ...
    if [ $? -eq 0 ]; then
        ...
        sudo apt-get update
        sudo apt install --assume-yes winehq-staging libgnutls30:i386 libldap-2.4-2:i386 libgpg-error0:i386 libxml2:i386 libasound2-plugins:i386 libsdl2-2.0-0:i386 libfreetype6:i386 libdbus-1-3:i386 libsqlite3-0:i386 kubuntu-restricted-extras -f
        sudo apt install -f --assume-yes --no-install-recommends dialog dosbox steam lutris q4wine winetricks
    fi
    which pacman >/dev/null 2>&1
    if [ $? -eq 0 ]; then
