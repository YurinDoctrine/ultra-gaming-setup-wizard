...
nvidia() {
    ...
    which pacman >/dev/null 2>&1
    if [ $? -eq 0 ]; then
        yay -S --needed --noconfirm mesa nvidia nvidia-390xx-dkms nvidia-390xx-utils opencl-nvidia lib32-opencl-nvidia lib32-nvidia-utils xf86-video-nouveau libva-mesa-driver linux-firmware
    fi
}
