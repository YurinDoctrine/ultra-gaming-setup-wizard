...
nvidia() {
    ...
    if [ $? -eq 0 ]; then
        ...
        sudo apt-get update
        sudo apt install -y --no-install-recommends libgl1-mesa-dri libglx-mesa0 libglx-mesa0:i386 mesa-vulkan-drivers xserver-xorg-video-all nvidia-driver-450 libnvidia-gl-450 libnvidia-gl-450:i386 libvulkan1 libvulkan1:i386 
    fi
    which pacman >/dev/null 2>&1
    if [ $? -eq 0 ]; then
...
prompt_2() {
    ...
    if [ $? -eq 0 ]; then
        ...
        sudo apt-get update
        sudo apt install winehq-staging libgnutls30:i386 libldap-2.4-2:i386 libgpg-error0:i386 libxml2:i386 libasound2-plugins:i386 libsdl2-2.0-0:i386 libfreetype6:i386 libdbus-1-3:i386 libsqlite3-0:i386 -y
        sudo apt install -y --no-install-recommends build-essential manpages-dev libx11-dev xorg-dev dialog dbus steam lutris
    fi
    which pacman >/dev/null 2>&1
    if [ $? -eq 0 ]; then
        ...
        yay -S --needed --noconfirm dialog dbus steam lutris
    fi
}
prompt_2
